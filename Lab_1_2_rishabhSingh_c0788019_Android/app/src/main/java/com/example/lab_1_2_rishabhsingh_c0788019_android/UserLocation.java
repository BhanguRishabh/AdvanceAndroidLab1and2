...
import com.example.lab_1_2_rishabhsingh_c0788019_android.databinding.ActivityUserLocationBinding;
...
public class UserLocation extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        Double recivedLat = getIntent().getDoubleExtra("lat",0);
        Double recievedLong = getIntent().getDoubleExtra("lon",0);


        mMap = googleMap;

        // Add a marker in Sydney and move the camera
        LatLng sydney = new LatLng(recivedLat, recivedLat);
